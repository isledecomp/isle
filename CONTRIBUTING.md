...
One important aspect of this compiler is that it keeps hidden bookkeeping state.
...
This still matters when contributing: a harmless-looking edit can move other
compiled output. Use [`reccmp`](/tools/README.md) while iterating and let the
ReproBit CI check the complete exact build. The project video explains the
problem visually [starting at 9:03](https://youtu.be/gthm-0Av93Q?t=543s).
