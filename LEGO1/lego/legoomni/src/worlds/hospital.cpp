...
Hospital::~Hospital()
{
	...
	m_hospitalState->m_state = HospitalState::States::e_unknown3;
...
MxResult Hospital::Create(MxDSAction& p_dsAction)
{
	...
	if (!m_hospitalState) {
		...
		m_hospitalState->m_state = HospitalState::States::e_newState;
	}
	else if (m_hospitalState->m_state == HospitalState::States::e_unknown4) {
		m_hospitalState->m_state = HospitalState::States::e_unknown4;
	}
	else {
		m_hospitalState->m_state = HospitalState::States::e_unknown3;
	}
...
void Hospital::ReadyWorld()
{
	...
	if (m_hospitalState->m_stateActor < 3) {
		...
		m_hospitalState->m_state = HospitalState::States::e_introduction;
		...
		m_hospitalState->m_state = HospitalState::States::e_explainQuestShort;
...
MxLong Hospital::HandleKeyPress(MxS8 p_key)
{
	...
	if (p_key == VK_SPACE && g_deleteCounter == 0) {
...
MxLong Hospital::HandleEndAction(MxEndActionNotificationParam& p_param)
{
	...
	case HospitalState::States::e_introduction:
		m_hospitalState->m_state = HospitalState::States::e_explainQuest;
		...
		g_deleteCounter = 0;
		break;
	case HospitalState::States::e_explainQuestShort:
		m_time = Timer()->GetTime();
		m_interactionMode = 1;
		break;
	case HospitalState::States::e_explainQuest:
	case HospitalState::States::e_unknown10:
		m_hospitalState->m_state = HospitalState::States::e_waitAcceptingQuest;
		m_interactionMode = 1;
		m_time = Timer()->GetTime();
		break;
	case HospitalState::States::e_unknown11:
		switch (m_currentActorId) {
		case LegoActor::c_pepper:
			switch (m_hospitalState->m_statePepper) {
			case 0:
			case 1:
				m_hospitalState->m_state = HospitalState::States::e_afterAcceptingQuest;
		...
		break;
	case HospitalState::States::e_afterAcceptingQuest:
		m_hospitalState->m_state = HospitalState::States::e_beforeEnteringAmbulance;
		act1State = (Act1State*) GameState()->GetState("Act1State");
		act1State->SetUnknown18(9);
	case HospitalState::States::e_exitViaDoor:
		if (m_unk0x128 == 0) {
			m_unk0x128 = 1;
		...
		break;
	case HospitalState::States::e_exitViaInfo:
		if (m_unk0x128 == 0) {
			m_unk0x128 = 1;
	...
	if (m_interactionMode == 1) {
		...
		if (roi != NULL) {
			...
			if (!strcmpi("actor_ha", roiName)) {
				...
				if (m_hospitalState->m_state == HospitalState::States::e_explainQuestShort) {
					if (m_unk0x128 == 0) {
						m_unk0x128 = 1;
				...
				else if (m_hospitalState->m_state == HospitalState::States::e_unknown10 || m_hospitalState->m_state == HospitalState::States::e_waitAcceptingQuest) {
					if (m_hospitalState->m_state == HospitalState::States::e_unknown10) {
						m_hospitalState->m_state = HospitalState::States::e_unknown11;
...
MxBool Hospital::HandleControl(LegoControlManagerNotificationParam& p_param)
{
	if (p_param.m_unk0x28 == 1) {
		...
		case HospitalScript::c_Info_Ctl:
			...
			if (m_interactionMode == 1) {
				m_hospitalState->m_state = HospitalState::States::e_exitViaInfo;
				...
				m_unk0x128 = 1;
				m_hospitalState->m_state = HospitalState::States::e_exitImmediately;
				m_destLocation = LegoGameState::e_infomain;
...
MxBool Hospital::Escape()
{
	...
	m_hospitalState->m_state = HospitalState::States::e_exitToClose;
