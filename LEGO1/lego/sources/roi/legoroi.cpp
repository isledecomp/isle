...
void LegoROI::DoNothingWithError(const LegoChar* p_error, const LegoChar* p_name)
{
}
...
LegoResult LegoROI::Animate(LegoTreeNode* p_node, const Matrix4& p_matrix, LegoTime p_time, LegoROI* p_roi)
{
	MxMatrix mat;
	...
	if (roi != NULL) {
		...
		for (LegoU32 i = 0; i < p_node->GetNumChildren(); i++) {
			Animate(p_node->GetChild(i), roi->m_local2world, p_time, roi);
		}
	}
	else {
		DoNothingWithError("%s ROI Not found\n", name);
	}
...
LegoResult LegoROI::SetFrame(LegoAnim* p_anim, LegoTime p_time)
{
	...
	return Animate(root, mat, p_time, this);
}
